...
# 读取yam配置
filename = './configs/CPFN/pointpillars_attention.yaml'
...
class PointAttentionChannelAttentionLayer(nn.Module):
    ...
    def __init__(self, dim_ca, dim_pa, reduction_r):
        super(PointAttentionChannelAttentionLayer, self).__init__()
        # 点注意力
        self.pa = PointAttentionLayer(dim_pa, dim_pa // reduction_r)
        # 通道注意力
        self.ca = ChannelAttentionLayer(dim_ca, dim_ca // reduction_r)
        self.sig = nn.Sigmoid()
...
class VoxelAttentionLayer(nn.Module):
    '''
    Voxel-wise attention for each voxel
    '''

    def __init__(self, c_num, p_num):
        super(VoxelAttentionLayer, self).__init__()
    ...
    def forward(self, voxel_center, paca_feat):
        ...
        voxel_feat_concat = torch.cat(
            [paca_feat, voxel_center_repeat], dim=-1)  # K,N,C---> K,N,(C+3)
...
class VoxelFeature_PPA(nn.Module):
    ...
    def __init__(
            ...
            dim_pa=cfg.PPA.NUM_POINTS_IN_VOXEL,
            reduction_r=cfg.PPA.REDUCTION_R,
            boost_c_dim=cfg.PPA.BOOST_C_DIM,
            use_paca_weight=cfg.PPA.USE_PACA_WEIGHT):
        super(VoxelFeature_PPA, self).__init__()
        self.PACALayer1 = PointAttentionChannelAttentionLayer(
            dim_ca=dim_ca, dim_pa=dim_pa, reduction_r=reduction_r)
        self.PACALayer2 = PointAttentionChannelAttentionLayer(
            dim_ca=boost_c_dim, dim_pa=dim_pa, reduction_r=reduction_r)
        self.voxel_attention1 = VoxelAttentionLayer(c_num=dim_ca, p_num=dim_pa)
        self.voxel_attention2 = VoxelAttentionLayer(
            c_num=boost_c_dim, p_num=dim_pa)
...
class PillarFeature_PPANet(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.name = 'PillarFeature_PPANet'
        ...
        num_input_features = cfg.PPA.BOOST_C_DIM

        # Create PillarFeatureNet layers
        num_filters = [num_input_features] + list(num_filters)

        self.VoxelFeature_TA = VoxelFeature_PPA()
        pfn_layers = []
        ...
        # Need pillar (voxel) size and x/y offset in order to calculate pillar
        # offset
...
class PSA(nn.Module):
    ...
    def forward(self, x, bev=None):
        ...
        PSA_output = torch.cat(
            [concat_conv1, concat_conv2, concat_conv3], dim=1)
        ...
        if self._use_direction_classifier:
            ...
            refine_dir_preds = refine_dir_preds.permute(
                0, 2, 3, 1).contiguous()
